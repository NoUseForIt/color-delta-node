...
import { useUploadCCT, useAnalyzeColors, useCorrectColors, useExportCCT } from '../hooks/useColorAPI';
...
const ColorDashboard = () => {
  ...
  const analyzeMutation = useAnalyzeColors();
  const correctMutation = useCorrectColors();
  const exportMutation = useExportCCT();

  const [activeTab, setActiveTab] = useState('original');

  // Auto-analyse après upload
  useEffect(() => {
    if (colors.length > 0 && whitePoint && !analysis) {
      analyzeMutation.mutate({ colors, whitePoint });
    }
  }, [colors, whitePoint, analysis]);
  ...
  const handleCorrect = () => {
    if (!analysis || !whitePoint) return;

    correctMutation.mutate({
      colors,
      whitePoint,
      targetWhitePoint: analysis.whitePointDrift.target,
      deltaWP: analysis.whitePointDrift.deltaE,
    });
  };
  ...
  const handleExport = () => {
    ...
    exportMutation.mutate({
      ...
      metadata: {
        ...
        source: fileName,
        corrected: correctedColors.length > 0,
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    disabled={!analysis || isLoading || analysis.whitePointDrift.deltaE < 1}
